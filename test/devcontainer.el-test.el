...
(defmacro expect-comint-process (cmdargs &rest body)
  (declare (indent 1))
...
(ert-deftest container-up-devcontainer-needed-secrets-file-non-existant ()
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((devcontainer-startup-secrets-file ".secrets.json"))
      (expect-comint-process `("up"
                               "--docker-path" "/path/to/docker"
                               "--workspace-folder" ,(file-name-as-directory real-project-root-dir))
        (devcontainer-up)
...
(ert-deftest container-up-devcontainer-needed-secrets-file-absolute-existant ()
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((devcontainer-startup-secrets-file (concat (make-temp-file "home" 'directory) ".secrets.json")))
...
(ert-deftest execute-command-container-up-whitespace-strip ()
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((stdout-buf (generate-new-buffer "*DevC foo command*"))
          (cli `("/some/path/devcontainer" "exec" "--docker-path" "/path/to/docker"
                     "--workspace-folder" ,(file-name-as-directory real-project-root-dir)
...
(ert-deftest compile-start-advice-devcontainer-up-no-term-environment ()
  (devcontainer-mode 1)
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((advice-string "docker exec --workdir /workspaces/the-project/ --user vscode --env PATH=/home/vscode/bin --env FOO=to\\ be\\ masked cdefab")
          (advice-list '("docker" "exec" "--workdir" "/workspaces/the-project/" "--user" "vscode" "--env" "PATH=/home/vscode/bin" "--env" "FOO=to be masked" "cdefab"))
                         )
      (mocker-let ((my-compile-fun (command &rest rest) ((:input `(,(concat advice-string " my-command foo")))))
                   ...
                   (devcontainer-remote-user () ((:output "vscode")))
        ...
        (should (equal (devcontainer-advice 'in-terminal)
                       '("docker" "exec" "-it" "--workdir" "/workspaces/the-project/" "--user" "vscode" "--env" "PATH=/home/vscode/bin" "--env" "FOO=to be masked" "cdefab")))
...
(ert-deftest devcontainer-advice-devcontainer-up-with-term-environment ()
  ...
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((devcontainer-term-environment '(("FOO" . "foo") ("BAR" . "bar bar")))
          (advice '("docker" "exec" "--workdir" "/workspaces/the-project/" "--user" "vscode" "--env" "PATH=/home/vscode/bin" "cdefab")))
      (mocker-let ((devcontainer-remote-workdir () ((:output "/workspaces/the-project/")))
        ...
        (should (equal (devcontainer-advice 'in-terminal)
                       '("docker" "exec" "-it" "--workdir" "/workspaces/the-project/" "--user" "vscode" "--env" "PATH=/home/vscode/bin" "--env" "FOO=foo" "--env" "BAR=bar bar" "cdefab")))))))

(ert-deftest compilation-start-no-exclude-simple-docker ()
  (devcontainer-mode 1)
  (fixture-tmp-dir "test-repo-devcontainer"
    (let ((cmd "docker exec --workdir /workspaces/project/ --user myuser --env PATH=/usr/bin abcdef grep foo")
      ...
      (mocker-let ((my-compile-fun (command &rest rest) ((:input `(,cmd))))
                   ...
                   (devcontainer-remote-user () ((:output "myuser")))
...
(ert-deftest compilation-start-no-exclude-simple-podman ()
  ...
  (let ((devcontainer-engine 'podman))
   (fixture-tmp-dir "test-repo-devcontainer"
     (let ((cmd "podman exec --workdir /workspaces/project/ --user myuser --env PATH=/usr/bin abcdef grep foo")
           (devcontainer-execute-outside-container nil))
       (mocker-let ((my-compile-fun (command &rest rest) ((:input `(,cmd))))
                    (devcontainer-remote-workdir () ((:output "/workspaces/project/")))
                    (devcontainer-remote-user () ((:output "myuser")))
